...
class OPS implements Runnable
{
 ...
 public void run()
 {
  ...
  {
   sum=0;
   for(j=1;j<=dim;j++)sum+=data[i][j];
   sum=(1-sum)/dim;
   for(j=1;j<=dim;j++)data[i][dim+j]=data[i][j]+sum;
  }
 }
 ...
 public static void main(String args[]) throws Exception
 {
  ...
  Scanner scan;
  if(args.length<2){System.out.println("Usage: java OPS <no_obj> <no_dim> <file-name> <no_threads_to_create>");return;}
  try
  {
  ...
  catch(Exception e){System.out.println("Usage: java OPS <no_obj> <no_dim> <file-name> <no_threads_to_create>");return;}
  if(args.length>=3)
  ...
  //Scanning data and entropy is also calculated
  for(i=0;i<no_obj;i++)
  {
   sum=0;
   for(j=0;j<dim+1;j++)
  ...
  for(i=0;i<cores;i++)
  {
   t[i]=new Thread(new OPS(i*no_obj/cores,(i+1)*no_obj/cores-1));
   t[i].start();
  }
